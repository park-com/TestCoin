...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(nBestHeight < 1)
    {
        ...
        else if(nBestHeight >= 1 && nBestHeight < YEAR_BLOCKS * 4)
    {
        nSubsidy = 20 * COIN;
    }
        else if(nBestHeight >= YEAR_BLOCKS * 4 && nBestHeight < YEAR_BLOCKS * 8)
    {
        nSubsidy = 10 * COIN;
    }
        else if(nBestHeight >= YEAR_BLOCKS * 8 && nBestHeight < YEAR_BLOCKS * 12)
    {
        nSubsidy = 5 * COIN;
    }
        else if(nBestHeight >= YEAR_BLOCKS * 12 && nBestHeight < YEAR_BLOCKS * 16)
    {
        nSubsidy = 2.50 * COIN;
    }
        else if(nBestHeight >= YEAR_BLOCKS * 16 && nBestHeight < YEAR_BLOCKS * 20)
    {
        nSubsidy = 1.25 * COIN;
    }
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    if(nBestHeight < YEAR_BLOCKS)	
    {
	nSubsidy = 4 * COIN_YEAR_REWARD * nCoinAge *33 / nDiv;
    }
    else if(nBestHeight < 2 * YEAR_BLOCKS)	
    {
	nSubsidy = 3 * COIN_YEAR_REWARD * nCoinAge * 33 / nDiv;
    }
    else if (nBestHeight < 3 * YEAR_BLOCKS)
    {
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "soomcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "2017-11-26 Politics: Jared Kushner’s Vast Duties, and Visibility in White House, Shrink";  // pskbank
        CTransaction txNew;
        txNew.nTime = 1511676091;  //pskbank
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1511676091;   // pskbank
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 1590123 : 1590123;  // pskbank
                    ...
                    while (true)
                       {
                          ...
                          {
                              fprintf(stderr,"nonce %08X: hash = %s (target = %s)\n", block.nNonce, block.GetHash().ToString().c_str(), block.hashMerkleRoot.ToString().c_str());
                          }
        ...
        assert(block.hashMerkleRoot == uint256("0x12346375baf4ad5a598f8b33352029c24a3121aa0bc57f65561d43f4b4a7b3e0"));  // pskbank
...
unsigned char pchMessageStart[4] = { 0x0d, 0x45, 0x83, 0xb0 };
