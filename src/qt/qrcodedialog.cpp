...
{
    QString ret = QString("soomcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SZC unit input in lnReqAmount, we generate the URI with SZC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::SZC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
